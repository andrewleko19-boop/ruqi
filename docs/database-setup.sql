...
-- ═════════════════════════════════════════════════════════════════════════════
-- §19 — إنهاء تخزين كلمات المرور نصّاً صريحاً في admin_credentials
-- ═════════════════════════════════════════════════════════════════════════════
-- كان العمود `password` يحتفظ بكلمة المرور مقروءة، وكانت البوابتان (المديرية
-- والوزارة) تعرضانها على الشاشة. لم تعد أي واجهة تقرأه، وإجراء reset_password
-- في الدالة الطرفية صار **يمحوه** (يكتب NULL) بعد توليد كلمة جديدة يعرضها مرة
-- واحدة فقط. فيلزم:
--   • السماح بـ NULL على العمود حتى يمكن محوه.
--   • عمود updated_at لتسجيل آخر تدوير.
--
-- ملاحظة مقصودة: لا يوجد هنا أمر مسح جماعي للصفوف القائمة. مسحها دفعةً واحدة
-- يمحو كلمات مرور ربما لم تُسلَّم لأصحابها بعد، فيقفل حسابات عاملة. النصّ
-- الصريح يُمحى لكل حساب لحظة تدوير كلمته من زرّ «إنشاء كلمة مرور جديدة».
-- إن أردت المسح الفوري للجميع (بعد التأكّد من تسليم الجميع) نفّذ يدوياً:
...
-- ═══════════════════════════════════════════════════════════════════════════
-- §26  نظام "الوحدات القابلة للتفعيل" (Module Permissions) — تفعيل تدريجي بالدور
-- ═══════════════════════════════════════════════════════════════════════════
-- طبقة عرض (UI rollout) فقط، إضافية بالكامل: لا تعديل على current_user_role()
-- أو أي RLS policy قائمة. إخفاء وحدة = إخفاء زرّها/تبويبها في الواجهة، لا حجب
-- استعلاماتها أو RPCs الخاصة بها في الخلفية — البيانات والعلاقات تبقى متّصلة.
--
-- جداول جديدة: modules (فهرس الوحدات)، role_module_permissions (مصفوفة دور×وحدة).
-- عمود جديد: users.permission_role — مستوى إداري فرعي ضمن نفس role التقني
-- (مثلاً "الوزير"/"موظف الوزارة" ضمن ministry_user)، الحسابات القائمة تُهجَّر
-- تلقائياً إلى المستوى الأعلى فلا يفقد أحد وصولاً كان يملكه.
...
-- ── ٢٦.٣  عمود المستوى الإداري الفرعي ────────────────────────────────────────
...
-- ── ٢٦.٤  دالة حلّ المستوى الفعّال (تُعالج ولي الأمر خصيصاً — لا صف له في users) ──
...
-- ── ٢٦.٥  دالة الوحدات المفعّلة للمستخدم الحالي ──────────────────────────────
...
-- ── ٢٦.٦  تحديث updated_by/updated_at تلقائياً عند كل تبديل من لوحة التحكم ───
...
-- ── ٢٦.٧  تعبئة فهرس الوحدات (14 وحدة أعمال) ─────────────────────────────────
...
-- ── ٢٦.٨  تعبئة مصفوفة الأدوار × الوحدات ─────────────────────────────────────
-- الحزمة الأولى: مفعّلة بحسب الملاءمة. المراحل التالية: مخفية للجميع افتراضياً
-- (dense seed — كل الصفوف موجودة من اليوم الأول لتكون المصفوفة شفافة بالكامل).
