...
(async () => {
  ...
  if (!data) {
    // «لا شيء» تعني: لا جلاءَ مرتبطاً بهذا الرمز في السجلّ — وهذا يقع أيضاً حين
    // تكون الورقة بطاقةً مدرسية داخلية طُبعت قبل أن تُرسل المدرسة جلاء الصفّ إلى
    // المديرية. فلو قلنا «لا توجد شهادة» وحدها لقرأها الماسح تزويراً، فيُتَّهم
    // طالبٌ بريء بورقةٍ سليمة. نقول ما نعرفه بالضبط ولا نزيد.
    showError('لا يوجد جلاء مسجَّل مرتبط بهذا الرمز.',
      'إن كانت الوثيقة بطاقةً مدرسية داخلية فهذا متوقّع: التحقّق الرسميّ يعمل بعد إرسال المدرسة جلاء الصفّ إلى المديرية.');
    return;
  }
