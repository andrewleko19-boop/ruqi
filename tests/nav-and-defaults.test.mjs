...
describe('«الفصل الحاليّ» إعدادٌ حيّ لا زينة', () => {
  /* بُني الجدول والدالّة والمودال في لوحة المشرف، ثمّ بقيت البوّابتان اللتان
     تستعملان الفصل فعلاً تفتحان على أوّل خيار: منتقي «نوع الشهادة» في المدرسة
     يبدأ «السنة الكاملة» أبداً، و G.semester في المعلّم يبدأ 1 أبداً.
     فمعلّمٌ يُدخل درجات الفصل الثاني يكتبها في الأول إن لم ينتبه — ودرجاتٌ
     تُبنى عليها النتيجة النهائية. إعدادٌ لا يقرؤه أحد ليس إعداداً. */

  test('بوّابة المعلّم تقرأ الفصل الوطنيّ عند فتح شاشة الدرجات', () => {
    const src = read('teacher/script.js');
    assert.match(src, /getCurrentTerm,/, 'الدالّة غير مستورَدة — الإعداد لا يصل.');
    assert.match(src, /G\.semester = t === 's2' \? 2 : 1/);
  });

  test('واختيارُ المعلّم يعلو على الافتراض في جلسته', () => {
    const src = read('teacher/script.js');
    assert.match(src, /if \(!G\.semesterTouched\)/,
      'بلا رايةٍ يُدهَس اختيارُ المعلّم كلّما أُعيد فتح الشاشة.');
    assert.match(src, /G\.semesterTouched = true/);
  });

  test('بوّابة المدرسة تفتح «نوع الشهادة» على الفصل المُعلن', () => {
    const src = read('school/script.js');
    assert.match(src, /NDB\.getCurrentTerm\(\)/);
    assert.match(src, /repTermSelect\.value = term === 's1' \? 's1' : 'year'/);
    assert.match(src, /_repTermTouched/, 'بلا رايةٍ يُدهَس اختيار المدير.');
  });

  test('تعذُّرُ الجلب لا يُعطّل الشاشة — يبقى الظاهر', () => {
    // موضعُ **النداء** لا سطرُ الاستيراد: أوّل ظهورٍ للاسم هو الاستيراد،
    // والنافذةُ بعده لا تحوي catch فيُخطئ الفحص ويُحسب سليماً معطوباً.
    for (const [f, call] of [['teacher/script.js', 'await getCurrentTerm()'],
                             ['school/script.js',  'await NDB.getCurrentTerm()']]) {
      const src = read(f);
      const i = src.indexOf(call);
      assert.ok(i > 0, `${f}: لم يُعثر على نداء الفصل الحاليّ.`);
      assert.ok(/catch\s*\{/.test(src.slice(i, i + 400)),
        `${f}: جلبُ الفصل بلا catch — انقطاعُ الشبكة يُسقط فتح الشاشة.`);
    }
  });
});
