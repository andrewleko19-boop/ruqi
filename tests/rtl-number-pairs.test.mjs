...
describe('تنبيه النصاب يُقرأ كما هو', () => {
  ...
  /* الفارغ لم يعد حالةَ شخصٍ بل حالةَ إعداد: النصاب صار يُشتقّ من سلطته
     (تجاوزُ المدرسة ← الحدُّ الوطنيّ)، فإن لم تحدّده جهةٌ فالعيبُ في الإعداد
     لا في الكادر — رسالةٌ واحدة لا سطرٌ لكلّ موظّف. */
  test('«لم يُحدَّد نصاب» حالةُ إعدادٍ واحدة لا صفٌّ لكلّ موظّف', () => {
    assert.match(fn[0], /anyBound/,
      'غيابُ الحدّين يجب أن يُفحص مرّةً على الجدول كلِّه لا لكلّ صفّ.');
    assert.match(fn[0], /لم يُحدَّد نصابُ التدريس/,
      'الرسالة الصريحة غائبة — فيُخترَع رقمٌ أو يُتَّهم الكادر.');
  });

  test('النقصُ عن الحدّ الأدنى يُعرض كذلك — نصفُ الحدّ كان مهملاً', () => {
    assert.match(fn[0], /shortfall/,
      'الدالّة تُرجع حدّاً أدنى ولا أحد يقرؤه: معلّمٌ بأربع حصصٍ يمرّ صامتاً.');
    assert.match(fn[0], /نقص/, 'مقدار النقص بلا عنوان.');
  });
});
