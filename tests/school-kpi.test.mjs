...
describe('renderKpis — اليوم مكتمل الرصد', () => {
  ...
  /* صُحِّح بعد بلاغٍ من الميدان. كانت هذه الدالة وحدها تعدّ المعذور غائباً،
     بينما سجلّ الحضور أسفل الصفحة نفسها ولوحتا المديرية والوزارة تعدّه
     حاضراً — فظهر «١٩ حاضراً» فوق سجلٍّ يقول «٢٠». والغياب بعذرٍ ليس غياباً
     إدارياً: مُبرَّرٌ فلا يُحتسب على الطالب ولا يدخل إنذارات التسرّب. */
  test('المُعذَّر يُعدّ حاضراً — عذرُه يرفع عنه الغياب', () => {
    renderKpis([cls(10, 8, 1, 0, 1)]);
    assert.equal(val('kpi-absent'),  1, 'الغائب هو بلا عذرٍ وحده');
    assert.equal(val('kpi-present'), 9, 'المعذور يدخل في الحاضرين');
  });

  test('الاصطلاح موحَّد مع بقية التطبيق: حاضر+متأخّر+معذور', () => {
    // ٢٤ طالباً: ٢٠ حاضر، ١ متأخّر، ١ بعذر، ٢ غياب بلا عذر.
    renderKpis([cls(24, 20, 2, 1, 1)]);
    assert.equal(val('kpi-present'), 22);
